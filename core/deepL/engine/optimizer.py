...
def build_optimizer(model: Module, cfg) -> Optimizer:
    ...
    if optim_type == "SGD":
        ...
        optimizer = optim.AdamW(params, lr=cfg.optimizer.lr, weight_decay=cfg.optimizer.weight_decay, eps=cfg.optimizer.epsilon)
    else:
...
def build_scheduler(optimizer: Optimizer, cfg) -> _LRScheduler:
    ...
    if sched_type is None or sched_type == "None":
        ...
        scheduler = build_warmup_lr_scheduler(optimizer, cfg, annealing="linear")
                                                                                    ...
                                                                                    # epoch                   iter
        scheduler = optim.lr_scheduler.OneCycleLR(optimizer, cfg.optimizer.lr, (cfg.trainer.max_epoch * cfg.nums_train_dataset) + 100,
