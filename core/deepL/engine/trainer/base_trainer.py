...
from ....utils import (
    ensure_dir,
    Timer,
    get_logger, get_log_string,
    add_trainer_args, parse_args,
    get_config,
)
from ...tensor_ops import (
    move_to_cuda, tensor_to_array,
)
...
class BaseTrainer(abc.ABC):
    ...
    def __init__(self):
        ...
        self._resume = self._args.resume
        ...
        self._watch_model = self._args.watch_model
        ...
        # parse config
        self._cfg = cfg
        self._output_dir = cfg.experiment.output_dir
        self._checkpoint_dir = cfg.experiment.checkpoint_dir
        self._log_dir = cfg.experiment.log_dir
        self._manual_seed = cfg.experiment.seed + self._local_rank
        self._max_epoch = cfg.trainer.max_epoch
        self._grad_acc_steps = cfg.trainer.grad_acc_steps
        self._val_steps = cfg.trainer.val_steps

        # logger
        self._log_file = self._log_dir / "train.log"
        self._logger = get_logger(cfg=cfg,log_file=self._log_file)
        self.log(f"######$$$$$$$$$Number of GPUs detected by PyTorch: {str(torch.cuda.device_count())}")
        ...
        # find checkpoint
        self._checkpoint = self._args.checkpoint if self._args.checkpoint != "" else None
    ...
    def load(self, filename):
        if self._resume:
            ...
            """Load model only."""
            self._ckpt_manager.load_checkpoint(filename, self.model)
            self.log(f"Checkpoint filename: {filename}.")
    ...
    def register_model(self, model):
        ...
        if self._watch_model:
            self._logger.wandb_watch(model)
    ...
    def val_epoch(self):
        ...
        for iteration, data_dict in pbar:
            ...
            data_dict = self.before_val_step(self.epoch, self.iteration, data_dict)
            timer.toc("data")
            # val step
    ...
    def run(self, only_validation=False, debug=False):
        ...
        while self.epoch < self._max_epoch:
            ...
            if self.epoch % self._val_steps == 0:
                self.val_epoch()
            if self.epoch<=75:
                self.save("checkpoint.pth", save_training_states=True)
            else:
                self.save(f"epoch-{self.epoch}.pth", save_training_states=True)
